<template>
  <main class="auth-wrap">
    <form class="card" @submit.prevent="onSubmit">
      <h1>Loo konto</h1>
      <label class="lbl">Nimi (valikuline)</label>
      ...
      <button class="btn" :disabled="loading">{{ loading ? 'Loomine…' : 'Loo konto' }}</button>
      <p class="hint">On juba konto? <NuxtLink to="/login">Logi sisse</NuxtLink></p>
    </form>
  </main>
</template>
...
async function onSubmit() {
  ...
  loading.value = true
  try {
    const res = await register(email.value, password.value, name.value || undefined)
    if (!res?.ok) {
      error.value = res?.error || 'Konto loomine ebaõnnestus'
      return
    }
    router.push('/storage')
  } catch (e) {
